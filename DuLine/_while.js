...
module.exports = (text = [""], Com, line = 0) => {
    ...
    if (text[0] == "while") {
        if (text[1]) {
            ...
            if (end > -1) {
                ...
                try {
                    ...
                    if (err) {
                        Console.gerror("An unknown error in a logical expression.", ["From: DuScript interpreter.", "Worker: _while", "Line: " + line]);
                        work = false;
                    };
                };
            } else {
                Console.gerror("The logical expression must end with the \"::\" command.", ["From: DuScript interpreter.", "Worker: _while", "Line: " + line]);
                work = false;
            };
        } else {
        ...
        Console.gerror("A command was executed that was not invoked in the line.", ["From: DuScript interpreter.", "Worker: _while", "?: " + text.join(" "), "Line: " + line]);
        work = false;
    };
